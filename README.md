...
NAME
    ...
    zfold - small-file solid archive CLI for directory snapshots and backups

SYNOPSIS

    zfold <COMMAND> [OPTIONS]

DESCRIPTION

    zfold recursively scans a directory, groups files into solid chunks, compresses each chunk
    with zstd, and writes a single archive file.

    For a beginner-friendly overview in Japanese, see:
        docs/zfold-guide.ja.md

    During pack, completed chunks are written incrementally in chunk order. This keeps buffering
    bounded to in-flight work instead of the full archive.
...
PACK
    ...
    SYNOPSIS
        zfold pack <INPUT_DIR> -o <ARCHIVE>
            [--chunk-size <BYTES>]
            [--level <LEVEL>]
            [--threads <N>]
            [--dict <DICT_FILE>]
    ...
    OPTIONS
        ...
        --chunk-size <BYTES>
            ...
            Peak pack memory still depends on this value because each active worker assembles one
            uncompressed chunk in memory before compression.
        ...
        --threads <N>
            ...
            Higher values usually improve throughput, but peak pack memory also increases with
    ...
    NOTES
        ...
        pack writes to a temporary file and moves it into place after a successful run.
...
LIST
    ...
    NOTES
        If the archive is encrypted and no password source is specified, zfold prompts for the
...
EXAMPLES
    ...
    zfold pack ./dataset -o dataset.zpk --chunk-size 33554432 --level 8
    zfold pack ./dataset -o dataset.zpk --threads 1 --chunk-size 8388608
    zfold pack ./dataset -o dataset.zpk --dict app.dict --exclude "*.png"
    zfold pack ./dataset -o secure.zpk --password "correct horse battery staple"
    zfold pack ./dataset -o secure.zpk --password-env ZFOLD_PASSWORD
    zfold pack ./dataset -o secure.zpk --password-file ./archive.pass
    ...
    zfold extract dataset.zpk -d ./out --prefix "docs/"
    zfold verify dataset.zpk
    zfold verify secure.zpk --password "correct horse battery staple"
    zfold verify secure.zpk --password-env ZFOLD_PASSWORD
    zfold train-dict ./snap1 ./snap2 ./snap3 -o app.dict --mode text --extensions ini,json,txt
    zfold train-dict ./app -o app.dict --include "config/**" --exclude "*.png" --dict-size 65536
...
NOTES
    ...
    In low-memory environments, prefer a smaller --chunk-size and/or --threads 1.
